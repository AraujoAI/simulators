I have converted the content of your Google Docs document, "SIM Prompt Documentation," into a Markdown (`.md`) file format.

You can copy the code block below and save it as a file with the `.md` extension (e.g., `SIM_Prompt_Documentation.md`) for uploading to GitHub.

``` markdown
# SIM Prompt Documentation

## SIM Prompt Sheet

You are an Expert Frontend Developer

Task: Build a high-fidelity, self-contained HTML/CSS/JS simulator for training purposes based on the attached screenshots and the logic flow below.

**Requirements:**

1.  **Visuals:** Replicate the UI details from the screenshots exactly (colors, fonts, icon styles, and layout). If an element is in the image, it must be in the code.
2.  **Output:** A single valid HTML code block that I can run locally.
3.  **Logic:** Implement the workflow below:

**Global Goal**: Search for classes in My Innovator

**Image 1 + Image 2** (login page):

User selects the field where it says, “Username” and types mmiller
User selects the field where it says, “Password” and types innovator
Change the CTA button color state from gray to royal blue.
Once the royal blue CTA button is clicked, the next image appears

**Image 3**:

User selects the menu item called “> Training”

Create a corrective note for every menu item that gets clicked by accident. The pop-up message should be: “Look for Training”. Try again.

When the user selects “Training,” move the end user to the next screen with the Training drop down menu open.

**Image 4**:

User selects the menu item called “Classes”

If the end user selects another menu item from the drop down menu, re-dedirect them with, “Try again by looking for the word, Classes.”

**Image 5**:

The user selects the menu item called “Search Classes.” The next image should appear.

If the end user selects another menu item from the drop down menu, re-dedirect them with, “Try again by looking for “Search Classes.”

**Image 6**:

User selected the menu item called “Search Classes”

Once that menu item is selected, the image below will appear.
Create the ability that the “MyInnovator” logo can be selected for the end users to be redirected to the very first screen in this simulation.

**Last Step:**

Add a reset button that redirects the end user to start over.

Aras Corporation - Internal Use Only

## [TEMPLATE] SIM Prompt Sheet

SIM prompts to be used in Gemini
...
1.  **Visuals:** Replicate the UI details from the screenshots exactly (colors, fonts, icon styles, and layout). If an element is in the image, it must be in the code and therefore in the simulated environment.
2.  **Output:** A single valid HTML code block that I can run locally.
3.  **Logic:** Implement the workflow below:
...
`<iframe style="border:1px #FFFFFF none" src="https://xxxxxxxx.github.io/simulators/xxxxxxxx" title="iFrame" width="100%" height="600px" scrolling="no" frameborder="no" allow="fullscreen"></iframe>`
...
|                                                                                                                                                                                                                                                                     |
| :-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| **AI-POWERED** **SIMULATOR BUILD** **WORKFLOW**                                                                                                                                                                            |
|                                                                                                                                                                                                                                                                     |
| A step-by-step guide for instructional designers to use AI prompts to generate high-fidelity, interactive training simulators from screenshots. **ARAS LEARNING & DEVELOPMENT** Internal Use OnlyDo not copy, distribute, share or download without permission. |
...
Action: Build a high-fidelity, self-contained HTML/CSS/JS simulatorfor training purposes based on the attached screenshots and thelogic flow below. Context:1. Visuals: Replicate the UI details from the screenshots exactly   (colors, fonts, icon styles, and layout). If an element is in   the image, it must be in the code.2. Output: A single valid HTML code block that I can run locally.3. Logic: Implement the following workflow: Image 1 (\[NAME OF SCREEN\]): \[DESCRIBE ACTION\]Image 2 (\[NAME OF SCREEN\]): \[DESCRIBE ACTION\]Image 3 (\[NAME OF SCREEN\]): \[DESCRIBE ACTION\]
...
| | **Naming Example**01-Dashboard.png02-AddVendorForm.png03-ConfirmationMessage.png |
| :-: | :----------------------------------------------------------------------------------: |
...
> Image 4 (Confirmation): The confirmation dialog appears.Simulation ends here. Display a "Restart" button to allowthe learner to try again.
